 ...
 * @url https://gist.github.com/devmycloud/df28012101fbc55d8de1737762b70348
 *
 ...
 * 
 * Smpleader edited few lines
 *   - do not use Symfony\Component\HttpFoundation\File\UploadedFile
 *   - 
 *
 */
...
class ParseInputStream
{
    ...
    private function file_stream($data)
    {
		...
		$idx = strpos( $data, "\r\n\r\n" );
		if ( $idx === FALSE ) {
			Log::add( "ParseInputStream.file_stream(): Could not locate header separator in data:" );
			Log::add( $data );
		} else {
			...
			while ( $header !== FALSE ) {
				if ( substr($header, 0, strlen("Content-Disposition: ")) == "Content-Disposition: " ) {
					...
					Log::add( "PARSEINPUTSTREAM: Skipping Header: " . $header );
				}
			...
			if ( $bytes !== FALSE ) {
				//$file = new UploadedFile(  ,  UPLOAD_ERR_OK );
                $file = new UploadFile($filename, $path, $bytes, $filetype); 
	...
	function parse_parameter( &$params, $parameter, $value ) {
		if ( strpos($parameter, '[') !== FALSE ) {
			...
			if ( preg_match( '/^([^[]*)\[([^]]*)\](.*)$/', $parameter, $match ) ) {
				...
				Log::add( "ParseInputStream.parse_parameter() Parameter name regex failed: '" . $parameter . "'" );
			}
		} else {
